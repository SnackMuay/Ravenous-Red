...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    if (eggSpecies == (SPECIES_NIDORAN_F || (SPECIES_NIDORINA || SPECIES_NIDOQUEEN)) && daycare->offspringPersonality & EGG_GENDER_MALE)
        eggSpecies = SPECIES_NIDORAN_M;
    else if (eggSpecies == SPECIES_ILLUMISE && daycare->offspringPersonality & EGG_GENDER_MALE)
        eggSpecies = SPECIES_VOLBEAT;
    else if (P_NIDORAN_M_DITTO_BREED >= GEN_5 && eggSpecies == (SPECIES_NIDORAN_M || (SPECIES_NIDORINO || SPECIES_NIDOKING)) && !(daycare->offspringPersonality & EGG_GENDER_MALE))
